...
{
	int rResult = 0;
	rResult = RTMP264_Connect(&rtmp_PublishObj.rtmp,url);
...
void RtmpH264::DeletePublish()
{
	...
	if (m_isCreatePublish){
		RTMP264_Close(rtmp_PublishObj.rtmp);
	}
...
int RtmpH264::InitVideoParams(unsigned long width, unsigned long height, unsigned long fps, unsigned long bitrate, uint32_t pixelformat, bool bConstantsBitrate = false)
{
	...
	if (i_nal > 0){
		...
		InitMetaData(&rtmp_PublishObj.metaData,pps, pps_len, sps, sps_len, m_width, m_height, m_frameRate);
#if 0
...
int RtmpH264::SendScreenCapture(uint8_t * frame, unsigned long StrideHeight, unsigned long timespan, uint32_t pixelformat,int width, int height)
{
	...
	switch(pixelformat){
		...
		case V4L2_PIX_FMT_YUV420:{
			memcpy(y,frame,width*height);
			memcpy(u, frame + width*height, width*height / 4);
			memcpy(v, frame + width*height + width*height / 4, width*height / 4);
			break;
		}
		default:{
	...
	i_nal = 0;
	int ret = x264_encoder_encode(h, &nal_t, &i_nal, &m_picInput, &m_picOutput);
	...
	for (int i = 0; i < i_nal; i++)	{
		...
		if (nal_t[i].i_type == NAL_SLICE || nal_t[i].i_type == NAL_SLICE_IDR){
			...
			rResult = RTMP264_SendPacket(rtmp_PublishObj.rtmp, &rtmp_PublishObj.metaData,nal_t[i].p_payload + 4, nal_t[i].i_payload - 4, bKeyFrame, timespan);
		}
	}
	return rResult;
}
